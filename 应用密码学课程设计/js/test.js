...
function HightMod(iBottomNumber,iExponent,iMod){
    var iValueMessageAfterCompute=1;
    var iValueMessage=iBottomNumber;
    var iKey=iExponent;
    //把key以2进制的形式进行移位乘用
    while(iKey !== 0)
    {
        var t = iKey&1;
        if(t !== 0)
            iValueMessageAfterCompute=(iValueMessageAfterCompute*iValueMessage)%iMod;
        //高位右移
        iKey>>=1;
...
function Encryption(){
  ...
  var newChar;
    var newStr;
  for(var i=0; i<RSAIn.value.length; i++){
    ch = RSAIn.value[i];
    
    newChar = HightMod(parseInt(ch), E.value, N.value);
    // while (newChar.length < 8)
    //     newChar = '0' + newChar;
    newStr += newChar;
  }

RSAOut.innerHTML = newStr;
}
...
function product() {
 ...
 //循环至E与L互素
    while (true)
    {
        ...
        if ((E.value >= 2) && ExtendedEuclid(E.value, L))
            break;
    }
...
function test(){
  ...
  for(var i=0; i<RSAIn.value.length; i++){
   ...
   newChar = HightMod(parseInt(ch), E.value, N.value).toString(16);
    while (newChar.length < 8)
        newChar = '0' + newChar;
    newStr += newChar;
  }
...
function test1(){
  ...
  //   ch  = RSAIn.value[i];
  //   if (i > 7){
  //     newChar = HightMod(BASE64.encoder(s.toString(16)), D.value, N.value);
  ...
  //     i++;
  // }
      newChar = HightMod(parseInt(BASE64.encoder("123".toString(16))), D.value, N.value);
      newStr += newChar;
      RSAOut.innerHTML = newStr;
}
